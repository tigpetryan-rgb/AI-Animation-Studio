...
./gradlew \
  ...
  -PruntimeVersion="0.10.7-phase8-bim-upright-walk+${SOURCE_SHA:0:12}" \
...
adb shell am instrument -w -r \
  -e class com.aianimationstudio.runtime.NativePhase8BimUprightWalkInstrumentedTest \
...
if [[ "$INSTRUMENT_STATUS" -ne 0 ]] || grep -q -E 'FAILURES!!!|INSTRUMENTATION_FAILED|shortMsg=' "$RESULT_FILE" || ! grep -q 'OK (1 test)' "$RESULT_FILE"; then
  echo "Phase-8 Bim upright-walk instrumentation did not complete successfully." >&2
  cat "$RESULT_FILE" >&2
  exit 1
fi
cd ../..

rm -rf phase8-bim-upright-walk-artifact
mkdir -p phase8-bim-upright-walk-artifact
adb shell test -d "$REMOTE_DIR"
adb pull "$REMOTE_DIR/." phase8-bim-upright-walk-artifact/

FRAME_DIR="$(find phase8-bim-upright-walk-artifact -type d -name 'phase8-bim-upright-walk-frames' -print -quit)"
MANIFEST="$(find phase8-bim-upright-walk-artifact -type f -name 'phase8-bim-upright-walk-manifest.txt' -print -quit)"
...
echo "Native Bim upright-walk frame count: $FRAME_COUNT"
...
ROOT_MANIFEST="phase8-bim-upright-walk-artifact/phase8-bim-upright-walk-manifest.txt"
if [[ "$(realpath "$MANIFEST")" != "$(realpath -m "$ROOT_MANIFEST")" ]]; then cp "$MANIFEST" "$ROOT_MANIFEST"; fi
MP4="phase8-bim-upright-walk-artifact/phase8-bim-upright-walk-${SOURCE_SHA:0:12}.mp4"
WEBP="phase8-bim-upright-walk-artifact/phase8-bim-upright-walk-${SOURCE_SHA:0:12}.webp"
...
ffmpeg -y -hide_banner -loglevel error \
  -framerate 30 -start_number 0 -i "$FRAME_DIR/frame-%03d.png" \
  -c:v libwebp_anim -lossless 0 -q:v 80 -loop 0 -an "$WEBP"
...
cp "$FRAME_DIR/frame-000.png" phase8-bim-upright-walk-artifact/bim-upright-preview-1.png
cp "$FRAME_DIR/frame-075.png" phase8-bim-upright-walk-artifact/bim-upright-preview-2.png
cp "$FRAME_DIR/frame-150.png" phase8-bim-upright-walk-artifact/bim-upright-preview-3.png
cp "$FRAME_DIR/frame-225.png" phase8-bim-upright-walk-artifact/bim-upright-preview-4.png
cp "$FRAME_DIR/frame-299.png" phase8-bim-upright-walk-artifact/bim-upright-preview-5.png
rm -rf "$FRAME_DIR"
sha256sum "$MP4" "$WEBP" > phase8-bim-upright-walk-artifact/phase8-bim-upright-walk.sha256
